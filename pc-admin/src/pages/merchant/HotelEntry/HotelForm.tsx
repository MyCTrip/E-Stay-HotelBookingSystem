...
import { 
  ...
  HomeOutlined, TagOutlined, EnvironmentOutlined, 
  CloudUploadOutlined 
} from '@ant-design/icons'
import { 
  Button, Form, Input, InputNumber, Select, Upload, 
  Row, Col, Card, DatePicker, Space, message, Spin, Empty 
} from 'antd'
...
const HotelForm: React.FC = () => {
  ...
  if (fetching) {
      return <div style={{ textAlign: 'center', padding: 50 }}><Spin size="large" tip="加载数据中..." /></div>
  }
  ...
  return (
    <div style={{ maxWidth: '100%', margin: '0 auto' }}>
      ...
      >
        {/* === 模块一：基础信息 === */}
        <Card 
            title={<Space><HomeOutlined /><span>基础信息</span></Space>} 
            bordered={false} 
            style={{ marginBottom: 24, borderRadius: 8, boxShadow: '0 1px 2px rgba(0,0,0,0.03)' }}
        >
          <Row gutter={24}>
            <Col span={12}>
              <Form.Item label='酒店中文名' name='nameCn' rules={[{ required: true, message: '请输入酒店名称' }]}>
          ...
          <Row gutter={24}>
             <Col span={16}>
                <Form.Item label='详细地址' name='address' rules={[{ required: true, message: '请输入地址' }]}>
                    <Input prefix={<EnvironmentOutlined style={{color:'#bfbfbf'}}/>} placeholder='省/市/区/街道门牌号' />
                </Form.Item>
             </Col>
             <Col span={8}>
          ...
          <Form.Item name='images' label='酒店封面/相册' valuePropName='fileList' getValueFromEvent={normFile}>
            <Upload 
        ...
        <Card 
            ...
            bordered={false} 
            style={{ marginBottom: 24, borderRadius: 8, boxShadow: '0 1px 2px rgba(0,0,0,0.03)' }}
            extra={<Button type="dashed" size="small" onClick={() => {
                const rooms = form.getFieldValue('rooms') || []
                form.setFieldsValue({ rooms: [...rooms, { name: '', price: 200, stock: 10 }] })
            }} icon={<PlusOutlined />}>快捷添加</Button>}
        >
            <Form.List name="rooms">
            {(fields, { add, remove }) => (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                
                {fields.length === 0 && <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="暂无房型，请添加" />}
                ...
                {fields.map(({ key, name, ...restField }, index) => (
                    <Card 
                        ...
                        size="small"
                        title={`房型 #${index + 1}`}
                        extra={<Button type="text" danger icon={<MinusCircleOutlined />} onClick={() => remove(name)}>移除</Button>}
                    ...
                    <Row gutter={16}>
                        ...
                        <Col span={8}>
                        <Form.Item {...restField} name={[name, 'price']} label="单价 (元)" rules={[{ required: true, message: '必填' }]}>
                            <InputNumber min={0} style={{ width: '100%' }} prefix="￥" />
                        </Form.Item>
                        </Col>
                        <Col span={8}>
                        <Form.Item {...restField} name={[name, 'stock']} label="库存" rules={[{ required: true, message: '必填' }]}>
                ...
                <Button type="dashed" onClick={() => add()} block icon={<PlusOutlined />} style={{ height: 40, marginTop: 8 }}>
                    添加更多房型
                </Button>
                </div>
            )}
        ...
        <div style={{ textAlign: 'right', marginTop: 16 }}>
            <Space size="large">
                ...
                <Button htmlType='submit' type='primary' size='large' icon={<SaveOutlined />} loading={loading}>
