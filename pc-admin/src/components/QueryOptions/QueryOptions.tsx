...
interface QueryProps {
  query: { search: string; rows: string; sort: string }
  setQuery: (q: any) => void
  disabledSearch?: boolean
}

function QueryOptions({ query, setQuery, disabledSearch }: QueryProps) {
  return (
    <div className='flex flex-col items-center justify-between space-x-0 space-y-2 sm:flex-row sm:space-x-2 sm:space-y-0'>
      <Input
        className='space-x-4'
        onChange={(e) => setQuery((prev: any) => ({ ...prev, search: e.target.value }))}
      ...
      <Select
        ...
        onChange={(value) => setQuery((prev: any) => ({ ...prev, rows: value }))}
      ...
      <Select
        className='w-full sm:w-[240px]'
        onChange={(value) => setQuery((prev: any) => ({ ...prev, sort: value }))}
