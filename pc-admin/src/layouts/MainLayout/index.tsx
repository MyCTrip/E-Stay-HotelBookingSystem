...
function MainLayout() {
  ...
  let role = 'merchant'
  try {
    const userStr = localStorage.getItem('user') || localStorage.getItem('BEACH-RESORT-USER-STORAGE')
    if (userStr) {
  ...
  const activeKey = menuItems.find((m: any) => m.key && location.pathname.startsWith(m.key))?.key || ''
