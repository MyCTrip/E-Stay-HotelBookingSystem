...
export default function HomeHotelPage() {
  ...
  const cards = useMemo(
    () =>
      // 使用 (hotelList || []) 赋予默认空数组，确保 .map 永远安全执行
      (hotelList || []).map((hotel) => ({
        hotel,
        // 这里也加个安全防范，防止 roomSPUList 还没准备好
      ...
      <div className={styles.compactSearchSection}>
        <div className={styles.searchCard}>
          <div className={tabStyles.marketTabs}>
            <button
              type="button"
              className={`${tabStyles.marketTab} ${currentHotelMarket === 'domestic' ? tabStyles.active : ''
                }`}
            ...
            <button
              type="button"
              className={`${tabStyles.marketTab} ${currentHotelMarket === 'international' ? tabStyles.active : ''
                }`}
