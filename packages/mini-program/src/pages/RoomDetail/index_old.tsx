...
export default function RoomDetailPage() {
  ...
  if (!room) {
    return <View className={styles.error}>房间不存在或加载失败</View>
  }
        ...
        {room.headInfo && (
          <View className={styles.section}>
            ...
            <View className={styles.features}>
              ...
              <View className={styles.feature}>
                <Text className={styles.featureLabel}>{room.headInfo.wifi ? '📡' : '❌'} WiFi</Text>
                <Text className={styles.featureValue}>{room.headInfo.wifi ? '有' : '无'}</Text>
              </View>
              <View className={styles.feature}>
                <Text className={styles.featureLabel}>{room.headInfo.windowAvailable ? '🪟' : '❌'} 窗户</Text>
                <Text className={styles.featureValue}>{room.headInfo.windowAvailable ? '有' : '无'}</Text>
              </View>
              <View className={styles.feature}>
                <Text className={styles.featureLabel}>🚭 吸烟</Text>
                <Text className={styles.featureValue}>{room.headInfo.smokingAllowed ? '允许' : '不允许'}</Text>
              </View>
            </View>
          </View>
        )}
