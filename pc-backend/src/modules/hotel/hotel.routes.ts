...
// 创建酒店仅允许商户且必须为已通过商户
router.post('/', authenticate, requireRole('merchant'), requireMerchantVerified, validateBody(createHotelSchema), createHotel);
// 更新允许商户更新自身或管理员更新审核信息（控制逻辑在 controller）
router.put('/:id', authenticate, validateBody(updateHotelSchema), updateHotel);
// 提交审核仅允许商户且商户必须已通过
router.post('/:id/submit', authenticate, requireRole('merchant'), requireMerchantVerified, submitHotel);
