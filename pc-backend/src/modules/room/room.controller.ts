...
export const createRoom = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const room = await Room.create({ hotelId, baseInfo: baseInfo || {}, headInfo: headInfo || {}, bedInfo: bedInfo || [], breakfastInfo: breakfastInfo || {}, auditInfo: { status: 'draft' } });
...
export const updateRoom = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (updates.breakfastInfo) room.breakfastInfo = { ...room.breakfastInfo, ...updates.breakfastInfo };
    await room.save();
    res.json(room);
  } catch (err: any) {
    res.status(400).json({ message: err.message });
...
export const submitRoom = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (!hotel) return res.status(404).json({ message: 'Hotel not found' });
    if (hotel.merchantId.toString() !== user.id) return res.status(403).json({ message: 'Forbidden' });
    const updated = await Room.findByIdAndUpdate(id, { $set: { 'auditInfo.status': 'pending' } }, { new: true });
    if (!updated) return res.status(404).json({ message: 'Not found' });
    await AuditLog.create({ targetType: 'room', targetId: updated._id, action: 'submit', operatorId: user.id });
    res.json(updated);
  } catch (err: any) {
    res.status(400).json({ message: err.message });
  }
