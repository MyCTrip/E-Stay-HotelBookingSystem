...
async function run() {
  ...
  r = await http('/api/merchants/submit', {
    method: 'POST',
    headers: { Authorization: `Bearer ${merchantToken}` },
  });
  ...
  if (r.status !== 200) {
    ...
    log(
      'Ensure backend MONGO_URI matches test script MONGO_URI or set MONGO_URI env var when running the script.'
    );
  ...
  r = await http(`/api/admin/merchants/${merchantId}/approve`, {
    method: 'POST',
  ...
  r = await http(`/api/hotels/${hotelId}/submit`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${merchantToken}` },
  });
  ...
  r = await http(`/api/admin/hotels/${hotelId}/approve`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ reason: 'OK' }),
  });
  ...
  const roomPayload = {
    baseInfo: { type: 'Standard', price: 100, images: [], status: 'draft', maxOccupancy: 2 },
    headInfo: {
      size: '20 sqm',
      floor: '5',
      wifi: true,
  ...
  r = await http(`/api/hotels/${hotelId}/rooms`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${merchantToken}`, 'Content-Type': 'application/json' },
  ...
  r = await http(`/api/rooms/${roomId}/submit`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${merchantToken}` },
  });
  ...
  r = await http(`/api/admin/rooms/${roomId}/approve`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${adminToken}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ reason: 'OK' }),
  });
