...
// 将后端返回的结构转为表单值（勾选状态）
export const facilitiesToFormValues = (facilities: FacilityCategory[]): Record<string, string[]> => {
...
// 将表单值转为后端结构
export const formValuesToFacilities = (formValues: Record<string, string[]>): FacilityCategory[] => {
  return FACILITY_CATEGORIES.map((category) => ({
    ...
    facilities: category.facilities.map((facility) => ({
      id: facility.id,
      name: facility.name,
      available: formValues[category.id]?.includes(facility.id) ?? false,
    })),
  }))
}
