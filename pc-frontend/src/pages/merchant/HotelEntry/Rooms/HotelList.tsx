  ...
  const columns = [
    {
      ...
      render: (_: any, r: Hotel) => {
        ...
        return imgUrl ? <Avatar shape="square" size={64} src={imgUrl} /> : <div style={{width:64, height:64, background:'#f0f0f0', display:'flex', alignItems:'center', justifyContent:'center', borderRadius: 4}}>无图</div>
      }
    },
    { 
      title: '酒店名称', 
    ...
    { 
      title: '星级', 
      key: 'star',
      render: (_: any, r: Hotel) => <span style={{ color: '#faad14' }}>{'★'.repeat(r.baseInfo?.star || 0)}</span>
    },
    { 
      title: '状态', 
      key: 'status', 
    ...
    },
    {
      ...
      render: (_: any, r: Hotel) => (
        <Space>
          ...
          <Tooltip title="编辑信息">
            <Button type="primary" icon={<EditOutlined />} onClick={() => navigate(`/merchant/hotels/${r._id}/edit`)} />
          </Tooltip>
