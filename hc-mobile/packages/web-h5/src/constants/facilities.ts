...
// 兼容旧API - 获取指定分类中已有的设施
export const getFacilitiesInCategory = (categoryId: string, hasFacilities: string[] = []) => {
  const category = FACILITY_CATEGORIES.find((c) => c.id === categoryId)
  if (!category) return []
...
export const getFacilitiesWithStatus = (categoryId: string, hasFacilities: string[] = []) => {
  const category = FACILITY_CATEGORIES.find((c) => c.id === categoryId)
  if (!category) return []
