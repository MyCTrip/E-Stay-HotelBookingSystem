...
const HotelInfo: React.FC<HotelInfoProps> = ({ name, star, address, data }) => {
  const brand = '精选'
  ...
  const resolvedName = name ?? data?.baseInfo?.nameCn ?? '民宿名称'
  const resolvedStar = star ?? data?.baseInfo?.star ?? 0
  const resolvedAddress = address ?? data?.baseInfo?.address ?? '城市位置'

  const rating = Number.isFinite(resolvedStar) ? resolvedStar : 0
    ...
    <div className={styles.infoCard}>
      <div className={styles.headerRow}>
        <div className={styles.left}>
          <h1 className={styles.name}>{resolvedName}</h1>
